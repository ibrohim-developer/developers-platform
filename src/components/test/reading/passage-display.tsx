...
export function PassageDisplay({ title, content }: PassageDisplayProps) {
  ...
  // Initialize web-highlighter scoped to the content area
  useEffect(() => {
    ...
    highlighter.on(Highlighter.event.CLICK, ({ id }: { id: string }, _inst: Highlighter, e: Event) => {
  ...
  const handleMouseUp = useCallback(() => {
    ...
    const existingId = startEl && highlighterRef.current
  ...
  return (
    <article className="p-8 space-y-4 bg-white text-gray-900">
      ...
      {popup && (
        ...
        >
          <button
            onClick={popupMode === 'highlight' ? applyHighlight : removeHighlight}
